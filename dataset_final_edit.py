...
class CustomDataset_Pseudo(Dataset):
    ...
    def __init__(self, target, mean=(0.548, 0.504, 0.479), std=(0.237, 0.247, 0.246), val_ratio=0.2):
        ...
        self.test_data = pd.read_csv("csv/for_test_idx.csv")
    ...
    def setup(self):
        ...
        for i in range(len(self.test_data)):          
            ...
            img_path = os.path.join(self.test_dir, self.test_data.iloc[i]['filename'])  # (resized_data, 000004_male_Asian_54, mask1.jpg)
            mask_label = self.test_data.iloc[i]['mask']
            id = self.test_data.iloc[i]['filename']
            gender_label = self.test_data.iloc[i]['gender']
            age_label = self.test_data.iloc[i]['agegroup']
